...
const prepStops = (stops) => {
  ...
  return { colorArr: arr, colorCount: count };
};
...
const GradientBlinds = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      dpr: dpr !== undefined ? dpr : (typeof window !== 'undefined' ? window.devicePixelRatio : 1),
      alpha: true,
      antialias: true,
    });
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if (uLightMode > 0.5) {
        ...
        float pigmentMax = max(max(pigment.r, max(pigment.g, pigment.b)), 0.0001);
        pigment = mix(pigment / pigmentMax, (pigment / pigmentMax) * (pigment / pigmentMax), 0.12) * 0.72;
...
`;
    ...
    const { colorArr, colorCount } = prepStops(gradientColors);
    const uniforms = {
    ...
    // Listen on window so cursor tracking works even when canvas has pointer-events:none
  ...
  }, [dpr, paused, gradientColors, angle, noise, blindCount, blindMinWidth, mouseDampening, mirrorGradient, spotlightRadius, spotlightSoftness, spotlightOpacity, distortAmount, shineDirection, lightMode]);
